const intervalDivPoints = setInterval(() => {
  const divBottomChatTwitch = getElementByXpath(
    "//div[@class='chat-input__buttons-container tw-flex tw-justify-content-between tw-mg-t-1']"
  );
  if (divBottomChatTwitch) {
    if (!getElementByXpath("//span[@id='span-number-points-receiver']")) {
      const divPoints = document.createElement("div");
      divPoints.classList.add(
        "tw-inline-flex",
        "tw-relative",
        "tw-tooltip-wrapper",
        "revert-position"
      ...
      switch (detectBrowser()) {
        case "chrome":
          icon.classList.add("icon", "icon-chrome");
          break;
        case "mozilla":
          icon.classList.add("icon", "icon-mozilla");
          break;
      }
      ...
      divTip.classList.add(
        "tw-tooltip",
        "tw-tooltip--align-center",
        "tw-tooltip--up"
      );
...
const intervalPStreamer = setInterval(() => {
  const pStreamer = getElementByXpath(
    "//p[@class='tw-c-text-inherit tw-font-size-5 tw-white-space-nowrap']"
  );
  if (pStreamer) {
    compareStreamers(pStreamer.innerHTML);
    getElementByXpath(
      "//span[@id='span-number-points-receiver']"
    ).innerHTML = localStorage.getItem("gg_pointsReceivedToday");
  }
}, 3000);

const intervalConfigTwitch = setInterval(() => {
  const divConfigTwitch = getElementByXpath(
  ...
  if (divConfigTwitch) {
    if (!getElementByXpath("//p[@id='GetGiftTitleId']")) {
      ...
      labelSoundName.classList.add(
        "tw-drop-down-menu-input-item__label",
        "tw-flex-grow-1",
        "tw-mg-r-2"
      );
      ...
      inputToggleSound.checked =
        localStorage.getItem("gg_soundCheck") == "true" ? true : false;
      ...
      labelInputSoundName.classList.add(
        "label-input-sound-name",
        "tw-toggle__button"
      );
...
const intervalClickButtonGift = setInterval(() => {
  const btnGift = getElementByXpath(
  ...
  const intervalValuePointReceived = setInterval(() => {
    valuePointReceived = getElementByXpath(
      "//div[@class='tw-align-items-center tw-c-text-link tw-flex tw-pd-y-05']"
    );
    if (valuePointReceived) {
      clearInterval(intervalValuePointReceived);
      totalPointsReceived =
        parseInt(localStorage.getItem("gg_pointsReceivedToday")) +
        parseInt(valuePointReceived.innerText.split("+")[1]);
...
function audioPlay() {
  if (localStorage.getItem("gg_soundCheck") == "true") {
    switch (detectBrowser()) {
      case "chrome":
        var audio = new Audio(chrome.runtime.getURL("assets/beep.mp3"));
        break;
      case "mozilla":
        var audio = new Audio(browser.runtime.getURL("assets/beep.mp3"));
        break;
    }
    audio.play();
  }
